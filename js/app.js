(() => {
    ...
    function initSliders() {
        if (document.querySelector(".materials__slider")) new core(".materials__slider", {
        ...
        let projectMaxSlider = null;
        let projectMinSlider = null;
        if (document.querySelector(".projectmax__slider")) projectMaxSlider = new core(".projectmax__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
            ...
            watchOverflow: false,
            navigation: {
                prevEl: ".projectmax__prev",
                nextEl: ".projectmax__next"
            },
            keyboard: {
                enabled: true,
                onlyInViewport: true,
        ...
        if (document.querySelector(".projectmin__slider")) projectMinSlider = new core(".projectmin__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
        ...
        if (projectMaxSlider && projectMinSlider) {
            projectMaxSlider.on("slideChange", (() => {
                const i = projectMaxSlider.realIndex;
                projectMinSlider.slideTo(i);
                if (window.projectpopupimgs) projectpopupimgs.slideTo(i);
            }));
            projectMinSlider.on("slideChange", (() => {
                const i = projectMinSlider.realIndex;
                projectMaxSlider.slideTo(i);
                if (window.projectpopupimgs) projectpopupimgs.slideTo(i);
            }));
        }
    }
    ...
    da.init();
    document.querySelectorAll(".videobox").forEach((video => {
        video.addEventListener("click", (function() {
            if (video.classList.contains("ready")) return;
            let src = video.dataset.src;
            video.classList.add("ready");
            video.innerHTML = "<span>Загрузка...</span>";
            if (video.classList.contains("videoboxRuTube")) video.insertAdjacentHTML("afterbegin", `<iframe width="100%" height="100%" src="https://rutube.ru/play/embed/${src}/?autoplay=true" \n         frameBorder="0" allow="clipboard-write; autoplay" \n         webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>`); else if (video.classList.contains("videoboxVK")) video.insertAdjacentHTML("afterbegin", `<iframe src="https://vk.com/video_ext.php?oid=-151366721&id=${src}&hd=2&autoplay=1" \n         width="100%" height="100%" \n         allow="autoplay; encrypted-media; fullscreen; picture-in-picture; screen-wake-lock;" \n         frameborder="0" allowfullscreen></iframe>`); else if (video.classList.contains("videoboxyoutube")) video.insertAdjacentHTML("afterbegin", `<iframe width="100%" height="100%" src="https://www.youtube.com/embed/YQHsXMglC9A?si=${src}&autoplay=1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>`);
        }));
    }));
    document.addEventListener("DOMContentLoaded", (function() {
        const portfolioItems = document.querySelectorAll(".portfolio_grid_item");
        portfolioItems.forEach((function(item) {
            const videoBlock = item.querySelector(".portfolio_grid_item_video");
            const imgBlock = item.querySelector(".portfolio_grid_item_img");
            if (!videoBlock) return;
            const dataSrcElement = videoBlock.querySelector("[data-src]");
            const hasDataSrc = dataSrcElement && "" !== dataSrcElement.getAttribute("data-src").trim();
            if (hasDataSrc) {
                if (imgBlock) imgBlock.style.display = "none";
                videoBlock.style.display = "";
            } else {
                videoBlock.style.display = "none";
                if (imgBlock) imgBlock.style.display = "";
            }
        }));
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const grid = document.querySelector(".portfolio_grid");
        const btn = document.querySelector(".portfolio_btn a");
        const items = grid?.querySelectorAll(".portfolio_grid_item");
        if (!grid || !btn || !items?.length) return;
        const CONFIGS = {
            1100: {
                rowHeight: 290,
                gap: 24,
        ...
        function getConfig() {
            if (window.innerWidth >= 1100) return CONFIGS[1100];
            if (window.innerWidth >= 850) return CONFIGS[850];
            if (window.innerWidth >= 450) return CONFIGS[450];
            return CONFIGS.default;
        }
        function calculateHeight(rows, config) {
            return rows * config.rowHeight + (rows - 1) * config.gap;
        }
        function toggleGrid(showAll = false) {
            const config = getConfig();
            const visibleItems = config.visibleRows * config.columns;
            if (showAll) {
                items.forEach((item => item.style.cssText = "position:static;opacity:1;visibility:visible"));
                setTimeout((() => grid.style.maxHeight = grid.scrollHeight + "px"), 10);
                btn.textContent = "Скрыть";
            } else {
                const visibleHeight = calculateHeight(config.visibleRows, config);
                grid.style.maxHeight = visibleHeight + "px";
                btn.textContent = "Показать больше";
                items.forEach(((item, i) => {
                    const shouldShow = i < visibleItems;
                    item.style.cssText = shouldShow ? "position:static;opacity:1;visibility:visible" : "position:absolute;opacity:0;visibility:hidden";
                }));
            }
        }
        grid.style.cssText = "transition:max-height 0.5s ease;overflow:hidden;position:relative";
        let expanded = false;
        ...
        window.addEventListener("resize", (() => {
            setTimeout((() => toggleGrid(expanded)), 250);
        }));
        toggleGrid(false);
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        document.querySelectorAll(".review_top_stars").forEach((block => {
            const stars = Math.max(0, Math.min(5, parseInt(block.dataset.stars, 10) || 0));
            const goldTpl = block.querySelector(".stargold");
            const grayTpl = block.querySelector(".stargray");
            if (!goldTpl || !grayTpl) return;
            block.innerHTML = "";
            for (let i = 0; i < 5; i++) {
                const img = (i < stars ? goldTpl : grayTpl).cloneNode(true);
                block.appendChild(img);
            }
        }));
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const MAX_HEIGHT = 14 * 1.4 * 3;
        const initReviews = () => {
            document.querySelectorAll(".review_boxtext").forEach((box => {
                if (box.dataset.inited) return;
                box.dataset.inited = "1";
                const text = box.querySelector(".review_text");
                if (!text) return;
                if (text.scrollHeight <= MAX_HEIGHT + 1) return;
                let expanded = false;
                text.style.maxHeight = MAX_HEIGHT + "px";
                text.style.overflow = "hidden";
                const btn = document.createElement("div");
                btn.className = "review_more";
                btn.textContent = "Показать отзыв";
                btn.addEventListener("click", (() => {
                    expanded = !expanded;
                    if (expanded) {
                        text.style.maxHeight = "none";
                        text.style.overflow = "visible";
                        btn.textContent = "Свернуть отзыв";
                    } else {
                        text.style.maxHeight = MAX_HEIGHT + "px";
                        text.style.overflow = "hidden";
                        btn.textContent = "Показать отзыв";
        ...
        setTimeout(initReviews, 2500);
        setTimeout(initReviews, 2500);
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const listItems = document.querySelectorAll(".ourwork_list .ourwork_li");
        const boxItems = document.querySelectorAll(".ourwork_right_items .ourwork_right_boxitem");
        if (!listItems.length || !boxItems.length) return;
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const btn = document.querySelector(".ourwork_btn");
        const container = document.querySelector(".ourwork_right_items");
        if (!btn || !container) return;
        const isMobile = () => window.innerWidth <= 850;
        const moveBtn = () => {
            if (!isMobile()) return;
            const activeBox = container.querySelector(".ourwork_right_boxitem.active");
            const target = activeBox?.querySelector(".ourwork_right_item_tt");
            if (target && btn.parentNode !== target) target.appendChild(btn);
        };
        setTimeout(moveBtn, 300);
        const observer = new MutationObserver(moveBtn);
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const categories = document.querySelectorAll(".catalog_body_top .category");
        const grids = document.querySelectorAll(".catalog_body_grid");
        const moreBtn = document.querySelector(".catalog_body_btn a");
        if (!categories.length || !grids.length || !moreBtn) return;
        categories.forEach((cat => {
            cat.addEventListener("click", (() => {
                const key = cat.dataset.category;
                const link = cat.dataset.categorylink;
                categories.forEach((c => c.classList.remove("active")));
                cat.classList.add("active");
                grids.forEach((g => g.classList.remove("active")));
                document.querySelector(`.catalog_body_grid[data-bodygrid="${key}"]`)?.classList.add("active");
                if (link) moreBtn.href = "/" + link;
            }));
        }));
    }));
    document.addEventListener("click", (e => {
        ...
        if (!btn) return;
        const value = btn.dataset.project?.trim() || btn.textContent.trim();
        const popup = document.querySelector(btn.dataset.popup);
        const input = popup?.querySelector(".boxfortitle input");
        if (input && value) input.value = value;
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const setVal = (q, val) => {
            const inp = document.querySelector(`.ques${q} input`);
            if (inp) inp.value = Array.isArray(val) ? val.join(", ") : val;
        };
        document.querySelectorAll(".kviz_q_choices.single").forEach((group => {
            const q = group.closest(".kviz_q").dataset.q;
            const active = group.querySelector("button.active");
            if (active) setVal(q, active.textContent.trim());
            group.addEventListener("click", (e => {
                if ("BUTTON" !== e.target.tagName) return;
                group.querySelectorAll("button").forEach((b => b.classList.remove("active")));
                e.target.classList.add("active");
                setVal(q, e.target.textContent.trim());
            }));
        }));
        document.querySelectorAll(".kviz_q_input input").forEach((inp => {
            const q = inp.closest(".kviz_q").dataset.q;
            setVal(q, inp.value || inp.placeholder);
            inp.addEventListener("input", (() => {
                setVal(q, inp.value || inp.placeholder);
            }));
        }));
        document.querySelectorAll(".kviz_q_choices.multi").forEach((group => {
            const q = group.closest(".kviz_q").dataset.q;
            const checks = [ ...group.querySelectorAll("input") ];
            checks.slice(0, 3).forEach((i => i.checked = true));
            setVal(q, checks.slice(0, 3).map((i => i.value)));
            group.addEventListener("change", (() => {
                const vals = checks.filter((i => i.checked)).map((i => i.value));
                setVal(q, vals);
            }));
        }));
        const kviz1 = document.querySelector(".popup__kviz1");
        const kviz2 = document.querySelector(".popup__kviz2");
        kviz1.querySelector(".btnorange").onclick = e => {
            e.preventDefault();
            kviz1.classList.remove("active");
            kviz2.classList.add("active");
        };
        kviz2.querySelector(".btntransparent").onclick = e => {
            e.preventDefault();
            kviz2.classList.remove("active");
            kviz1.classList.add("active");
        };
    }));
    window["FLS"] = false;
    menuInit();
